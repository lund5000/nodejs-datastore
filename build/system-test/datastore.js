...
mocha_1.describe('Datastore', () => {
    ...
    mocha_1.it('should allocate IDs', async () => {
        const keys = await datastore.allocateIds(datastore.key('Kind'), 10);
        assert.ok(keys);
    });
    mocha_1.describe('create, retrieve and delete', () => {
        const post = {
        ...
        mocha_1.it('should excludeFromIndexes correctly', async () => {
        ...
        mocha_1.it('should remove index with using wildcard in excludeFromIndexes', async () => {
            const longString = Buffer.alloc(1501, '.').toString();
        ...
        mocha_1.it('should auto remove index with excludeLargeProperties enabled', async () => {
            const longString = Buffer.alloc(1501, '.').toString();
        ...
        mocha_1.it('should accurately save/get a large int value via Datastore.int()', async () => {
            const postKey = datastore.key('Team');
            const largeIntValueAsString = '9223372036854775807';
            const points = src_1.Datastore.int(largeIntValueAsString);
        ...
        mocha_1.it('should wrap specified properties via IntegerTypeCastOptions.properties', async () => {
            const postKey = datastore.key('Scores');
            const largeIntValueAsString = '9223372036854775807';
            const panthers = src_1.Datastore.int(largeIntValueAsString);
        ...
        mocha_1.it('should save/get/delete with a key name', async () => {
            const postKey = datastore.key(['Post', 'post1']);
        ...
        mocha_1.it('should save/get/delete with a numeric key id', async () => {
        ...
        mocha_1.it('should save/get/delete a buffer', async () => {
        ...
        mocha_1.it('should save/get/delete with a generated key id', async () => {
            const postKey = datastore.key('Post');
            await datastore.save({ key: postKey, data: post });
        ...
        mocha_1.it('should save/get/update', async () => {
            const postKey = datastore.key('Post');
            await datastore.save({ key: postKey, data: post });
            const [entity] = await datastore.get(postKey);
        ...
        mocha_1.it('should save/get/merge', async () => {
        ...
        mocha_1.it('should save and get with a string ID', async () => {
        ...
        mocha_1.it('should fail explicitly set second insert on save', async () => {
            const postKey = datastore.key('Post');
            await datastore.save({ key: postKey, data: post });
            // The key's path should now be complete.
            assert(postKey.id);
        ...
        mocha_1.it('should fail explicitly set first update on save', async () => {
            const postKey = datastore.key('Post');
            await assertRejects(datastore.save({
                key: postKey,
                method: 'update',
        ...
        mocha_1.it('should save/get/delete multiple entities at once', async () => {
            const post2 = {
        ...
        mocha_1.it('should get multiple entities in a stream', done => {
            const key1 = datastore.key('Post');
            const key2 = datastore.key('Post');
            datastore.save([
                { key: key1, data: post },
        ...
        mocha_1.it('should save keys as a part of entity and query by key', async () => {
        ...
        mocha_1.describe('entity types', () => {
            mocha_1.it('should save and decode an int', async () => {
            ...
            mocha_1.it('should save and decode a double', async () => {
            ...
            mocha_1.it('should save and decode a geo point', async () => {
                const geoPointValue = {
                    latitude: 40.6894,
                    longitude: -74.0447,
                };
    ...
    mocha_1.describe('querying the datastore', () => {
        ...
        mocha_1.it('should limit queries', async () => {
            const q = datastore
        ...
        mocha_1.it('should not go over a limit', async () => {
            const limit = 3;
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
        ...
        mocha_1.it('should run a query as a stream', done => {
        ...
        mocha_1.it('should run a datastore query as a stream via query#runStream', done => {
            const q = datastore.createQuery('Character').hasAncestor(ancestor);
            let resultsReturned = 0;
            q.runStream()
                .on('error', done)
        ...
        mocha_1.it('should run a transaction query as a stream via query#runStream', done => {
        ...
        mocha_1.it('should not go over a limit with a stream', done => {
            const limit = 3;
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
        ...
        mocha_1.it('should filter queries with simple indexes', async () => {
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
        ...
        mocha_1.it('should filter queries with defined indexes', async () => {
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
                .filter('family', 'Stark')
        ...
        mocha_1.it('should filter by ancestor', async () => {
            const q = datastore.createQuery('Character').hasAncestor(ancestor);
            const [entities] = await datastore.runQuery(q);
            assert.strictEqual(entities.length, characters.length);
        });
        mocha_1.it('should filter by key', async () => {
        ...
        mocha_1.it('should order queries', async () => {
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
                .order('appearances');
        ...
        mocha_1.it('should select projections', async () => {
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
                .select(['name', 'family']);
        ...
        mocha_1.it('should paginate with offset and limit', async () => {
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
                .offset(2)
        ...
        mocha_1.it('should resume from a start cursor', async () => {
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
                .offset(2)
        ...
        mocha_1.it('should group queries', async () => {
            const q = datastore
                .createQuery('Character')
                .hasAncestor(ancestor)
                .groupBy('appearances');
        ...
        mocha_1.it('should query from the Query object', async () => {
            await datastore.createQuery('Character').run();
        });
    });
    mocha_1.describe('transactions', () => {
        mocha_1.it('should run in a transaction', async () => {
        ...
        mocha_1.it('should commit all saves and deletes at the end', async () => {
        ...
        mocha_1.it('should use the last modification to a key', async () => {
            const incompleteKey = datastore.key('Company');
            const key = datastore.key(['Company', 'Google']);
            const transaction = datastore.transaction();
            await transaction.run();
        ...
        mocha_1.it('should query within a transaction', async () => {
            const transaction = datastore.transaction();
            await transaction.run();
        ...
        mocha_1.it('should read in a readOnly transaction', async () => {
            const transaction = datastore.transaction({ readOnly: true });
            const key = datastore.key(['Company', 'Google']);
            await transaction.run();
            await transaction.get(key);
        });
        mocha_1.it('should not write in a readOnly transaction', async () => {
            const transaction = datastore.transaction({ readOnly: true });
            const key = datastore.key(['Company', 'Google']);
            await transaction.run();
            await transaction.get(key);
